...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!$db) {
        ...
        echo json_encode(array("error" => "데이터베이스 연결 실패."));
        exit;
    }
    mysqli_query($db, 'SET NAMES utf8');
    ...
    // 회원가입 성공 메시지 반환
    if ($result) {
        echo json_encode(array("message" => "회원가입이 성공적으로 완료되었습니다."));
    } else {
        http_response_code(500);
        echo json_encode(array("error" => "회원가입 중에 오류가 발생했습니다. 다시 시도해주시기 바랍니다."));
    }
} else {
    // POST 요청이 아닌 경우에는 오류 응답을 반환합니다.
    http_response_code(405); // Method Not Allowed
